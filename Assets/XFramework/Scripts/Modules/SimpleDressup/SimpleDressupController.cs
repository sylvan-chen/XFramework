...
namespace SimpleDressup
{
    ...
    public class SimpleDressupController : MonoBehaviour
    {
        ...
        private List<SkinnedMeshRenderer> _equipmentRenderers = new();
        ...
        public void SetDressupSlotFromGameObject(GameObject dressupObject, DressupSlotType slotType)
        {
            ...
            if (dressupObject.TryGetComponent<SkinnedMeshRenderer>(out var renderer))
            {
                _equipmentRenderers.Add(renderer);
            }
        ...
        private async UniTask<bool> CombineMeshesAsync()
        {
            ...
            Log.Debug($"[SimpleDressupController] Meshes combined successfully - {combineInstances.Count} instances (including {_bodyRenderers?.Length ?? 0} original body parts).");
        ...
        private bool ApplyToTargetRenderer()
        {
            ...
            try
            {
                ...
                foreach (var renderer in _equipmentRenderers)
                {
                    if (renderer != null)
                    {
                        renderer.enabled = false; // 禁用原始装备渲染器
                    }
                }
                foreach (var renderer in _bodyRenderers)
                {
                    if (renderer != null)
                    {
                        renderer.enabled = false; // 禁用原始角色部件渲染器
                    }
                }
        ...
        public void CollectDressupSlotsFromChildren()
        {
            ...
            foreach (var renderer in renderers)
            {
                ...
                if (_bodyRenderers != null && _bodyRenderers.Contains(renderer)) continue;
